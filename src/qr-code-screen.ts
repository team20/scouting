...
export class QrCodeScreen extends LitElement {
    ...
    render() {
        return html`
            <div id="qr-container">
                <vaadin-button @click=${this.renderQRCode}>Display QR Code</vaadin-button><br>
                <canvas ${ref(this.canvas)} style="grid-column: 3; background-color: white; width: 500px; height: 500px;"></canvas>
                <vaadin-button ${ref(this.sessionRestart)} @click=${this.restartSession}>Restart Session</vaadin-button>
                <label>
                    Continue Scouting?
                    <vaadin-checkbox
